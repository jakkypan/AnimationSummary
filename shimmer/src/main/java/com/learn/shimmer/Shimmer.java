...
public class Shimmer {
    ...
    public static final int ANIMATION_DIRECTION_TTB = 2;
    public static final int ANIMATION_DIRECTION_BTT = 3;
    ...
    public Shimmer setDirection(int direction) {
        ...
        if (direction != ANIMATION_DIRECTION_LTR && direction != ANIMATION_DIRECTION_RTL
                && direction != ANIMATION_DIRECTION_TTB && direction != ANIMATION_DIRECTION_BTT) {
    ...
    public <V extends View & ShimmerViewBase> void start(final V shimmerView) {
        ...
        final Runnable animate = new Runnable() {
            ...
            public void run() {
                ...
                if (shimmerView.isVertical()) {
                    float fromY = 0;
                    float toY = shimmerView.getHeight();
                    if (direction == ANIMATION_DIRECTION_BTT) {
                        fromY = shimmerView.getHeight();
                        toY = 0;
                    }

                    animator = ObjectAnimator.ofFloat(shimmerView, "gradientY", fromY, toY);
                } else {
                    float fromX = 0;
